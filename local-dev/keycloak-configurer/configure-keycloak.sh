...
configure_keycloak() {
  ...
  MANAGE_USER_ROLE_ID=$(http --ignore-stdin -A bearer -a "$KEYCLOAK_ADMIN_ACCESS_TOKEN" "http://user-tasks-keycloak:8888/admin/realms/$REALM_NAME/ui-ext/available-roles/users/$SERVICE_ACCOUNT?first=0&max=11&search=manage-user" | jq -r ".[0].id")

   echo "Adding manage-users role to Client"
   http --ignore-stdin -q -A bearer -a "$KEYCLOAK_ADMIN_ACCESS_TOKEN" POST "http://user-tasks-keycloak:8888/admin/realms/$REALM_NAME/users/$SERVICE_ACCOUNT/role-mappings/clients/$REAL_MANAGEMENT_CLIENT_ID" \
   ...
   http --ignore-stdin -q -A bearer -a "$KEYCLOAK_ADMIN_ACCESS_TOKEN" POST "http://user-tasks-keycloak:8888/admin/realms/$REALM_NAME/roles" \
   ...
   http --ignore-stdin -b -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" POST "http://user-tasks-keycloak:8888/admin/realms/$REALM_NAME/users" \
           emailVerified:=true \
           username="my-user" \

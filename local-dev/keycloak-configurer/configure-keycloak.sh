...
configure_keycloak() {
    echo "Proceeding to create lh realm in keycloak and the client for user-tasks-bridge as sample OIDC provider"
...
#   Here we fetch Keycloak's admin access token. This access token will be used in all of the subsequent Http requests
    ...
    if http --ignore-stdin -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" ${KEYCLOAK_URL}/admin/realms/${REALM_NAME} -q --check-status; then
        echo "$REALM_NAME realm already exists, not recreating. Exiting success"
        exit 0
    fi
...
#  Here we create a custom claim that is also added to the accessToken. This custom claim is used by the lh-user-tasks-bridge-api
...
#  Here we create a role that will later on be used to identify admin users.
   echo "Creating User Tasks Bridge Admin Role"
   http --ignore-stdin -q -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" POST "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/roles" \
    ...
    description="This role is used to let User Tasks Bridge API know about which users are allowed to perform ADMIN actions."

#  Here we create a user that will not have the admin role.
   echo "Creating User Tasks Bridge Standard User"
...
#  Here we create a user that will have the admin role.
   echo "Creating User Tasks Bridge Admin User"
   http --ignore-stdin -b -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" POST "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/users" \
               emailVerified:=true \
   ...
   MANAGE_REALM_ROLE_ID=$(http --ignore-stdin -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/clients/${REALM_MANAGEMENT_CLIENT_ID}/roles/manage-realm" | jq -r ".id")
...
# The manage-realm role, among other things, allows admin users to see which users have roles assigned to them
   echo "Assigning manage-realm Role to Admin User"
   http --ignore-stdin -b -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" POST "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/users/${ADMIN_USER_ID}/role-mappings/clients/${REALM_MANAGEMENT_CLIENT_ID}" \
              [0][id]="$MANAGE_REALM_ROLE_ID" \
              [0][name]="manage-realm"
...
# Here we create groups
   ...
   http --ignore-stdin -q -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" POST "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/groups" name="admins"
   ADMINS_GROUP_ID=$(http --ignore-stdin -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/groups?first=0&max=1&exact=true&global=true&search=admins" | jq -r ".[0].id")
   http --ignore-stdin -q -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" PUT "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/users/${ADMIN_USER_ID}/groups/${ADMINS_GROUP_ID}"

   echo "Creating users group"
   http --ignore-stdin -q -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" POST "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/groups" name="users"
   USERS_GROUP_ID=$(http --ignore-stdin -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/groups?first=0&max=1&exact=true&global=true&search=users" | jq -r ".[0].id")
   http --ignore-stdin -q -A bearer -a "${KEYCLOAK_ADMIN_ACCESS_TOKEN}" PUT "${KEYCLOAK_URL}/admin/realms/${REALM_NAME}/users/${NON_ADMIN_USER_ID}/groups/${USERS_GROUP_ID}"

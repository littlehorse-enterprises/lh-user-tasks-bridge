...
export class BadRequestError extends LHUserTasksError {
  constructor(message: string = 'Bad request - The request was malformed or contained invalid parameters') {
    super(message);
  }
}

export class UnauthorizedError extends LHUserTasksError {
  constructor(message: string = 'Unauthorized - Authentication is required to access this resource') {
    super(message);
  }
}
...
export class NotFoundError extends LHUserTasksError {
  constructor(message: string = 'Not found - The requested resource does not exist') {
    super(message);
  }
}

export class PreconditionFailedError extends LHUserTasksError {
  constructor(message: string = 'Precondition failed - The request cannot be completed in the current state') {
    super(message);
  }
}
...
export class ValidationError extends LHUserTasksError {
  constructor(message: string = 'Validation error - The provided data is invalid') {
    super(message);
  }
}

export class TaskStateError extends LHUserTasksError {
  constructor(message: string = 'Task state error - The task is in an invalid state for this operation') {
    super(message);
  }
}

export class AssignmentError extends LHUserTasksError {
  constructor(message: string = 'Assignment error - The task cannot be assigned in its current state') {
    super(message);
  }
}

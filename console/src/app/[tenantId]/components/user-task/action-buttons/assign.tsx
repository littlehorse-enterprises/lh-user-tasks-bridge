...
export default function AssignUserTaskButton({
  ...
  const [firstSelection, setFirstSelection] = useState<'none' | 'user' | 'group'>('none');
  ...
  useEffect(() => {
    ...
    const filteredGroups = allGroups.filter(group => userGroupIds.includes(group.id));
    
    console.log(`User ${selectedUser.username} belongs to ${userGroupIds.length} groups`);
  ...
  useEffect(() => {
    if (!selectedGroupId || firstSelection !== 'group' || allGroups.length === 0) {
      return;
    }
    
    console.log(`Filtering users for group ${selectedGroupId}, first selection: ${firstSelection}`);
    
    // Filter users to only include those who belong to the selected group
    const usersInGroup = allUsers.filter(user => {
      const isInGroup = (user.groups || []).some(g => g.id === selectedGroupId);
      if (isInGroup) {
    ...
    console.log(`Found ${usersInGroup.length} users in group ${selectedGroupId}`);
    ...
    if (selectedUserId) {
      ...
      if (!isUserInGroup) {
        console.log(`Deselecting user ${selectedUserId} as they're not in group ${selectedGroupId}`);
          ...
          <div className="space-y-4">
            ...
            <div>
              <Label htmlFor="user">
                ...
                {firstSelection === 'group' && availableUsers.length < allUsers.length && (
            ...
            <div>
              <Label htmlFor="group">
                ...
                {firstSelection === 'user' && availableGroups.length < allGroups.length && (
                  <span className="ml-1 text-sm text-blue-500">
                    (filtered by user)
                  </span>
                )}
            ...
            {firstSelection !== 'none' && (
              <div className="text-sm text-muted-foreground">
                <p>
                  {firstSelection === 'user' ? (
                    <>You selected a user first. Groups are filtered to only show groups this user belongs to.</>
                  ) : (
                    <>You selected a group first. Users are filtered to only show users who belong to this group.</>
                  )}
                </p>
              </div>
            )}

...
import { ListUserTasksResponse, UserGroup } from "@littlehorse-enterprises/user-tasks-bridge-api-client";
    ...
    <div>
      ...
      <Tabs defaultValue={userGroups[0].id}>
        ...
        {userGroups.map((userGroup) => {
          ...
          if (isLoading) return <Loader2Icon className="size-4 animate-spin" key={userGroup.id} />;
          if (error) return <div key={userGroup.id}>Error loading tasks: {error.message}</div>;
